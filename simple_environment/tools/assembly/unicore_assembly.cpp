...
unsigned long get_inst_binary(Instruction &inst)
{
...
void produce_assembly(char filename[])
{
	FILE *p=fopen(filename,"r");
	if(p==NULL){
		cout <<filename<<" is not exist!"<<endl;
	...
	fclose(p);
	fclose(w);
	cout <<"produce successfully -> "<<dstname<<endl;
}
...
int main(int argv, char **args)
{
	...
	if(argv<2){
		...
		cout <<"./run unicore.asm"<<endl;
		//read_inst_template("d:\\unicore_templates.txt");
		//produce_assembly("d:\\temp.asm");
		return 0;
	}
